...
if __name__ == "__main__":
    ...
    df = preprocess("./test-final.csv", to_drop=to_drop)
    test_X = df.iloc[:, :-1].values.tolist()
    test_y = df.iloc[:, -1].values.tolist()

    # Create classifier
    tree = TreeClassifier()
    tree.fit(train_X, train_y)
    predictions = [tree.estimate(x) for x in test_X]
    print(
        "Accuracy: ",
        sum([1 for x, y in zip(predictions, test_y) if x == y]) / len(test_y),
    )
